...
#+begin_src elisp
  ;; 获取开发机的 ip
  (defvar my-remote-machine-ip nil)
  (defun my-get-remote-machine-ip ()
    ""
    (make-process
     :name "remote machine ip"
     :command '("ssh" "xhcoding@192.168.1.2" "ip addr show eth0")
     :filter (lambda (_ data)
               (when (string-match "inet \\(.*\\)/" data)
                 (setq my-remote-machine-ip
                       (substring data (match-beginning 1) (match-end 1))))))
    nil)

  (my-get-remote-machine-ip)
