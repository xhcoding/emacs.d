...
#+begin_src elisp
  (defconst my-rsync-ssh-path "c:/Users/xhcoding/scoop/apps/cwrsync/current/bin/ssh.exe")

  (defun my-rsync-files-command (src dst ip port user pass)
    "The command of Sync files from src to user@ip:dst with rsync."
     (when-let* ((cygdrive-src (replace-regexp-in-string "\\([cd]\\):" "/cygdrive/\\1" (expand-file-name src)))
                 (rsync-cmd (format "sshpass -p %s rsync -avPz -e '%s -p %s' %s %s@%s:%s" pass my-rsync-ssh-path port cygdrive-src user ip dst)))
       rsync-cmd))
#+end_src
...
#+begin_src elisp
  (use-package projectile
    ...
    (defun my-project-rsync-command()
      "Rsync command."
      (when-let* ((src (my-project-root))
                  (dst "/home/xhcoding/")
                  (ip "192.168.139.128")
                  (port "22")
                  (user "xhcoding")
                  (pass "xiaohuan"))
        (my-rsync-files-command src dst ip port user pass)))
...
#+begin_src elisp
  (use-package c-ts-mode
    :hook ((c-ts-mode c++-ts-mode) . my-cc-enable-dev)
    :custom
...
#+begin_src elisp
  (use-package org
    ...
    (create-fontset-from-fontset-spec
     (font-xlfd-name
      (font-spec :family "Sarasa Mono HC"
                 :weight 'regular
                 :slant 'normal
...
#+begin_src elisp
  (use-package denote
    :straight t
    :custom
    (denote-directory (expand-file-name "denote" my-org-dir))
...
#+begin_src elisp
  ...
  (use-package aichat
    ...
    (setq aichat-bingai-proxy "127.0.0.1:7890"
          aichat-openai-proxy "127.0.0.1:7890")
    (aichat-toggle-debug)
