...
- [[#emacs-配置][emacs 配置]]
  ...
  - [[#常用的工具函数][常用的工具函数]]
  - [[#基本设置][基本设置]]
  - [[#使用-gcmh-优化垃圾回收][使用 gcmh 优化垃圾回收]]
  - [[#自动保存-buffer][自动保存 buffer]]
  - [[#rime-输入法][rime 输入法]]
  - [[#字体和主题][字体和主题]]
  - [[#图标字体安装][图标字体安装]]
  - [[#modeline-设置][modeline 设置]]
  - [[#hydra-方便的快捷键设置][hydra 方便的快捷键设置]]
  - [[#读取最近文件][读取最近文件]]
  - [[#更方便的弹出窗口][更方便的弹出窗口]]
  - [[#自动同步外部文件改变][自动同步外部文件改变]]
...
#+begin_src emacs-lisp
  (use-package elisp-mode
    :hook ((emacs-lisp-mode . my-enable-lsp-bridge)))
#+end_src
  ...
  The screenshot tool is determined by `org-download-screenshot-method'."
      ...
      (let* ((screenshot-dir (file-name-directory org-download-screenshot-file))
             (org-download-screenshot-file
              (expand-file-name (concat (file-name-sans-extension (file-name-nondirectory (buffer-file-name))) ".png") screenshot-dir)))
    ...
    (defun my-org-download-method (link)
      (let ((filename
            ...
            (dirname (expand-file-name (file-name-sans-extension (file-name-nondirectory       (buffer-file-name))) my-blog-img-dir)))
...
#+begin_src elisp
  (use-package eaf
    ...
    (require 'eaf-git)
    (setq eaf-git-light-highlight-style "emacs")
    )
