...
#+begin_src elisp
  (use-package eshell
    ...
    (defun my-eshell-z--load ()
      (if (not (file-exists-p my-eshell-z-file-name))
          (setq my-eshell-z--table (make-hash-table :test 'equal))
        (setq my-eshell-z--table
              (with-temp-buffer
    ...
    (defun my-eshell-z--save ()
      (let ((dir (file-name-directory my-eshell-z-file-name)))
        ...
        (when (file-exists-p my-eshell-z-file-name)
          (copy-file my-eshell-z-file-name (concat my-eshell-z-file-name ".backup") t))
        (with-temp-file my-eshell-z-file-name
          (prin1 my-eshell-z--table (current-buffer)))))

    (defun my-eshell-z--update ()
      (let ((cur-dir default-directory))
        (if-let* ((item (gethash cur-dir my-eshell-z--table))
                  (score (car item)))
            (puthash cur-dir (list (+ score 1) (time-to-seconds)) my-eshell-z--table)
          (puthash cur-dir (list 1 (time-to-seconds)) my-eshell-z--table))))
           ...
           (t (let ((regex "")
                ...
                (maphash #'(lambda (key value)
                             (when (string-match-p regex key)
                               (when (and target-score (> (car value) target-score))
                                   (setq target-dir key
                                         target-score (car value)))
                               (unless target-score
                                 (setq target-dir key
                                       target-score (car value)))))
                         my-eshell-z--table)
