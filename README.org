...
#+begin_src elisp
  (use-package org
    ...
    (defun org-babel-execute:cmake-cpp (body params)
  ...
  Header arguments:
    ...
    :extra-src   - Named block(s) prepended to main source before compilation."
  	...
  	((listify
  	 ...
  	 (resolve-block
             (name)
             "Return the expanded source body of the named org block NAME.
  Must be called from the org buffer."
  	 ...
  	 (resolve-all-extra-srcs
             (parsed)
             "Resolve all extra source blocks while still in the org buffer.
  Returns a list of source strings."
             (mapcar #'resolve-block (plist-get parsed :extra-srcs)))

  	 (write-source
             (src-file extra-bodies)
             "Write assembled C++ source to SRC-FILE.
  EXTRA-BODIES is a list of already-resolved source strings."
  	 ...
  	 (run
             ...
             (org-babel-eval
              (concat (shell-quote-argument bin-file)
                      (let ((cmdline (plist-get parsed :cmdline)))
  	 ...
  	 (format-result
        ...
        (let* ((parsed      (parse-params))
               (extra-bodies (resolve-all-extra-srcs parsed))  ;; ← 在 org buffer 中解析
  	...
  	(unwind-protect
              (progn
                (write-source src-file extra-bodies)  ;; ← 传入已解析的字符串
                (write-cmakelists cmake-file parsed)
                (configure tmp-dir parsed)
                (build tmp-dir)
                (setq result (run bin-file parsed)))
