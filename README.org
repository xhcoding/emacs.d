...
#+begin_src elisp
  ...
  ;; 数据目录
  (defconst my-docs-dir (expand-file-name "Documents" my-dir-prefix))
...
#+begin_src elisp
  ...
  (use-package lsp-bridge
    ...
    (with-eval-after-load 'org
      (dolist (lang '("c" "cpp" "python" "rust" "denote-cpp"))
...
#+begin_src elisp
  ...
  ")
    (setq denote-excluded-directories-regexp "\\(.stfolder\\)")

    (defun my-denote-id()
      (format-time-string "%Y%m%dT%H%M%S"))

    (defun my/denote-insert-attach-name (&optional title)
      "Insert a Denote attachment filename at point.
  ID is generated by `format-time-string', TITLE is prompted."
      (interactive)
      (let* ((title    (or title (read-string "Title: ")))
             (filename (concat (my-denote-id) "--" title)))
        (insert filename)
        (message "Inserted: %s" filename)))

    (defun org-babel-execute:denote-cpp (body params)
      "Execute a C++ code block with CMake.
  BODY is the source code.  PARAMS is the header arguments alist.

  Header arguments:
    :pkgs        - Package(s) for find_package.
    :libs        - Library(ies) for target_link_libraries.
    :includes    - Include directory(ies).
    :defines     - Compile definition(s).
    :cmake-flags - Flags passed to cmake configure step.
    :flags       - Extra compiler flags (appended to CMAKE_CXX_FLAGS).
    :std         - C++ standard version (default \"20\").
    :cmdline     - Arguments passed to the compiled binary at runtime.
    :extra-src   - Named block(s) prepended to main source before compilation.
    :denote      - Denote id--title[.ext] string.  When set, the full destination
                   path is generated with org file keywords and passed as the last
                   command-line argument to the binary.  If the image file is
                   generated successfully, a denote: link is appended to the result.
                   Example: \"20250415T150233--depth-map-result.png\" or
                            \"20250415T150233--depth-map-result\" (no extension)."
      (cl-labels
  	((listify
             (val)
             (cond ((null val) nil)
  	 ...
  	 (parse-params
             ()
             (list :packages    (listify (cdr (assq :pkgs params)))
  		 :libs        (listify (cdr (assq :libs params)))
  		 :includes    (listify (cdr (assq :includes params)))
  		 :defines     (listify (cdr (assq :defines params)))
  		 :extra-srcs  (listify (cdr (assq :extra-src params)))
  		 :cmake-flags (cdr (assq :cmake-flags params))
  		 :flags       (cdr (assq :flags params))
  		 :std         (or (cdr (assq :std params)) "20")
  		 :cmdline     (cdr (assq :cmdline params))
  		 :denote      (cdr (assq :denote params))))

  	 ;; ---- org buffer helpers ----

  	 (resolve-block
             (name)
             (let ((info (org-babel-find-named-block name)))
               (unless info
                 (error "Named block '%s' not found" name))
               (save-excursion
                 (goto-char info)
                 (org-babel-expand-src-block))))

  	 (resolve-all-extra-srcs
             (parsed)
             (mapcar #'resolve-block (plist-get parsed :extra-srcs)))

  	 ;; ---- file I/O ----

  	 (write-source
             (src-file extra-bodies)
             (with-temp-file src-file
               (dolist (body-str extra-bodies)
                 (insert body-str)
                 (insert "\n"))
               (insert body)))
  	 ...
  	 (write-cmakelists
             ...
             (with-temp-file cmake-file
               (insert "cmake_minimum_required(VERSION 3.15)\n"
                       "project(org-babel-cmake)\n"
                       (format "set(CMAKE_CXX_STANDARD %s)\n"
                               (plist-get parsed :std))
                       "set(CMAKE_CXX_STANDARD_REQUIRED ON)\n"
                       "set(CMAKE_CXX_EXTENSIONS OFF)\n")
               (let ((flags (plist-get parsed :flags)))
                 (when flags
  		 (insert (format "set(CMAKE_CXX_FLAGS \"${CMAKE_CXX_FLAGS} %s\")\n"
  				 flags))))
               (dolist (pkg (plist-get parsed :packages))
                 (insert (format "find_package(%s REQUIRED)\n" pkg)))
               (dolist (inc (plist-get parsed :includes))
                 (insert (format "include_directories(%s)\n" inc)))
               (dolist (def (plist-get parsed :defines))
                 (insert (format "add_definitions(-D%s)\n" def)))
               (insert "add_executable(main main.cpp)\n")
               (dolist (lib (plist-get parsed :libs))
                 (insert (format "target_link_libraries(main %s)\n" lib)))))
  	 ...
  	 (configure
             ...
             (org-babel-eval
              (format "cmake -S %s -B %s %s"
                      ...
                      (shell-quote-argument tmp-dir)
                      (or (plist-get parsed :cmake-flags) ""))
              ""))

  	 (build
             (tmp-dir)
             (org-babel-eval
              (format "cmake --build %s -- -j$(nproc)"
                      (shell-quote-argument tmp-dir))
              ""))

  	 (run
             (bin-file parsed denote-path)
             (let ((cmdline (or (plist-get parsed :cmdline) "")))
               (when denote-path
                 (setq cmdline
                       (string-trim (concat cmdline " " denote-path))))
               (org-babel-eval
                (concat (shell-quote-argument bin-file)
                        (when (not (string-empty-p cmdline))
  			(concat " " cmdline)))
                "")))
  	 ...
  	 (parse-denote-name
             (denote-name)
             "Split DENOTE-NAME into (id . (title . ext)).
  DENOTE-NAME format: \"id--title\" or \"id--title.ext\".
  Example: \"20250415T150233--depth-map-result.png\"
    => (\"20250415T150233\" . (\"depth-map-result\" . \".png\"))
  Example: \"20250415T150233--depth-map-result\"
    => (\"20250415T150233\" . (\"depth-map-result\" . nil))"
             (let ((parts (split-string denote-name "--" t)))
               (if (>= (length parts) 2)
  		 (let* ((id    (car parts))
  			(rest  (mapconcat #'identity (cdr parts) "--"))
  			(ext   (file-name-extension rest t))
  			(title (file-name-sans-extension rest)))
                     (cons id (cons title (and ext (not (string-empty-p ext)) ext))))
                 (error "Invalid :denote format, expected id--title[.ext]: %s" denote-name))))

  	 (build-denote-dest-path
             (parsed)
             "Build destination path using `denote-format-file-name'.
  Returns (full-path . relative-path) or nil if :denote is not set."
             (when-let* ((denote-name (plist-get parsed :denote)))
               (let* ((org-dir    (get-org-dir))
                      (org-title  (or (get-org-file-title) "untitled"))
                      (keywords   (get-org-file-keywords))
                      (parsed-name (parse-denote-name denote-name))
                      (id         (car parsed-name))
                      (title      (cadr parsed-name))
                      (ext        (or (cddr parsed-name) ""))
                      (dest-dir   (file-name-as-directory
  				 (expand-file-name
  				  (concat "images/" org-title) org-dir)))
                      (denote-file (file-name-nondirectory
  				  (denote-format-file-name
                                     dest-dir id keywords title ext nil)))
                      (full-path  (expand-file-name denote-file dest-dir))
                      (rel-path   (concat "images/" org-title "/" denote-file)))
                 (cons full-path rel-path))))

  	 ;; ---- result formatting ----

  	 (make-denote-link
             (parsed)
             (let* ((parsed-name (parse-denote-name (plist-get parsed :denote)))
  		  (id    (car parsed-name))
  		  (title (cadr parsed-name)))
               (format "[[denote:%s]]" id)))

  	 (format-result
             (raw-result denote-paths parsed)
             (when raw-result
               (let ((result (org-remove-indentation raw-result)))
                 (setq result
  			...
  			(let ((tmp-file (org-babel-temp-file "c-")))
  			  ...
  			  (org-babel-import-elisp-from-file tmp-file)))
                 ...
                 (if (and denote-paths
  			(file-exists-p (car denote-paths)))
                     (concat result "\n" (make-denote-link parsed))
  		 result)))))

        ;; ---- main ----
        (let* ((parsed       (parse-params))
               (extra-bodies (resolve-all-extra-srcs parsed))
               (denote-paths (build-denote-dest-path parsed))
               (tmp-dir      (make-temp-file "ob-cmake-" t))
               (src-file     (expand-file-name "main.cpp" tmp-dir))
               (bin-file     (expand-file-name "main" tmp-dir))
               (cmake-file   (expand-file-name "CMakeLists.txt" tmp-dir))
               (result nil))
  	(unwind-protect
              (progn
                (write-source src-file extra-bodies)
                (write-cmakelists cmake-file parsed)
                (configure tmp-dir parsed)
                (build tmp-dir)
                (setq result
                      (run bin-file parsed
  			 (when denote-paths (car denote-paths)))))
            (delete-directory tmp-dir t))
  	(format-result result denote-paths parsed))))

    (eval-after-load 'org
      '(add-to-list 'org-src-lang-modes '("denote-cpp" . c++-ts)))


    (defun denote-image-insert (file)
      "Insert image file to notes."
      (let* ((blog-file-title (denote-retrieve-filename-title (buffer-file-name)))
             ...
             (blog-image-denote-id (my-denote-id))
             (blog-image-new-name (expand-file-name (concat blog-image-denote-id "--" blog-image-title blog-image-ext) blog-image-dir)))
        (unless (file-directory-p blog-image-dir)
          (make-directory blog-image-dir t))
        (message "Copy file from %s to %s" file blog-image-new-name)
        (copy-file file blog-image-new-name)
        (insert (format "[[denote:%s]]" blog-image-denote-id))))

    (defun denote/clipboard-image-insert()
      "Insert clipboard image."
      (interactive)
      (let* ((file (format "/tmp/clipboard-%s.png" (format-time-string "%s"))))
        (shell-command (format "xclip -selection clipboard -t image/png -o > %s" file))
        (unless (file-exists-p file)
          (error "save failed"))
        (denote-image-insert file)))
...
#+begin_src emacs-lisp
  (defconst my-hugo-root-dir (expand-file-name "Blog" my-docs-dir))
  ...
  (use-package ox-hugo
    ...
    ;; ~/Documents/Notes/images/1.png ==> https://images.xhcoding.cn/blog/1.png
    (defun my-hugo--image-path-to-url (raw-path)
      (let ((file-path (expand-file-name raw-path)))
        (concat my-hugo-image-url (string-trim-left file-path my-notes-image-dir))))

    (defun my-hugo--denote-blog-link-ol-export (link description format)
      ""
      (let* ((path-id (denote-link--ol-resolve-link-to-target link :path-id))
             (full-path (car path-id))
             (url))
        (if (my-hugo--blogs-image-path-p full-path)
            (let ((image-url (my-hugo--image-path-to-url full-path)))
              (message "image url: %s" image-url)
              (setq url (format "{{< figure src=\"%s\" >}}" image-url)))
          (let (path (file-relative-name (car path-id)))
            (setq url (format "[%s](%s)" description (concat my-hugo-post-url (file-name-sans-extension (downcase path)))))))
        url
        ))

    (with-eval-after-load 'org
      (org-link-set-parameters "denote" :export #'my-hugo--denote-blog-link-ol-export))

    (defun my-hugo*convert-denote-cpp-to-cpp (args)
    "将 ARGS 中 src-block 的 :language 从 denote-cpp 改为 cpp。"
    (let ((src-block (car args))
          (contents (cadr args))
          (info (caddr args)))
      ;; 复制 src-block 以避免副作用
      (setq src-block (org-element-copy src-block))
      (when (eq (org-element-type src-block) 'src-block)
        (let ((lang (org-element-property :language src-block)))
          (when (string= lang "denote-cpp")
            (org-element-put-property src-block :language "cpp"))))
      (list src-block contents info)))

    (advice-add #'org-hugo-src-block :filter-args #'my-hugo*convert-denote-cpp-to-cpp)
...
#+begin_src elisp
  (use-package tex
    :ban minimal?
    :hook (LaTeX-mode . my-enable-code-service)
    :straight (auctex)
    :defer t
    :config
